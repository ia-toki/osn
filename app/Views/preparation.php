<?= $this->extend('default') ?>

<?= $this->section('content') ?>
<h2>Persiapan</h2>
<p>Berikut ini adalah sumber-sumber yang dapat Anda manfaatkan untuk mempersiapkan diri menghadapi KSN-K, KSN-P, dan KSN bidang Informatika/Komputer, yang disusun oleh Ikatan Alumni TOKI.<p>
<p>Dengan ini, diharapkan seluruh siswa memiliki kesempatan yang sama dalam berkompetisi, terutama yang mungkin belum memiliki akses fasilitas seperti guru, senior yang pernah menjuarai olimpiade, ataupun dana untuk mengikuti pelatihan KSN.</p>
<hr />
<div class="bp3-card subcontent">
	<h3>Tingkat Kabupaten/Kota/Provinsi (KSN-K/KSN-P)</h3>
	<hr />
	<h4>Materi Matematika/Logika</h4>
        <p>Disarankan untuk mempelajari matematika tingkat SMA dan berlatih soal-soal KSN-K/KSN-P tahun-tahun sebelumnya.</p>
       
	<h4>Materi Pemrograman Dasar</h4>

	<table class="table table-striped table-bordered">
		<thead>
			<tr>
				<th class="col-preparation-name">Materi</th>
				<th>Bentuk</th>
				<th class="col-preparation-by">Oleh</th>
				<th>Catatan</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><a href="https://tlx.toki.id/courses/basic">Kursus Pemrograman Dasar TLX</a></td>
				<td>Kursus</td>
				<td>Ikatan Alumni TOKI</td>
				<td><p>Belajar algoritma dan pemrograman dasar dengan kurikulum yang disusun bertingkat, dan langsung latihan soal-soal melalui situs TLX.</p></td>
			</tr>
			<tr>
				<td><a href="https://www.youtube.com/playlist?list=PL42SmLrOBFuRoDXatrv1PajCbt2ejb2Gn">Seri Persiapan KSN Informatika: Pemrograman Dasar C++</a></td>
				<td>Video</td>
				<td>Ikatan Alumni TOKI</td>
				<td><p><i>Playlist</i> YouTube yang berisi video-video untuk memulai pemrograman dalam bahasa C++, mengikuti kurikulum kursus Pemrograman Dasar TLX.</p></td>
			</tr>
		</tbody>
	</table>

	<h4>Pembahasan Soal</h4>

	<table class="table table-striped table-bordered">
		<thead>
			<tr>
				<th class="col-preparation-name">Sumber</th>
				<th>Catatan</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><a href="https://www.kujawab.com/">Kujawab.com</a></td>
				<td><p>Tempat berdiskusi dan jawaban soal-soal OSK/OSP/OSN/KSN dari sesama pengguna.</p></td>
			</tr>
		</tbody>
	</table>
</div>

<div class="bp3-card subcontent">
	<h3>Tingkat Nasional (KSN)</h3>
	<hr />
	<h4>Materi Algoritma/Pemrograman Kompetitif</h4>

	<table class="table table-striped table-bordered">
		<thead>
			<tr>
				<th class="col-preparation-name">Materi</th>
				<th>Bentuk</th>
				<th>Bahasa</th>
				<th class="col-preparation-by">Oleh</th>
				<th>Catatan</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><a href="https://github.com/ia-toki/training-gate-id-pdf/archive/master.zip">Materi Kursus Pemrograman TLX</a></td>
				<td>Slide</td>
				<td>Indonesia</td>
				<td>Ikatan Alumni TOKI</td>
				<td><p>Materi pemrograman kompetitif dasar dan lanjutan.</p></td>
			</tr>
			<tr>
				<td><a href="https://tlx.toki.id/courses/competitive">Kursus Pemrograman Kompetitif Dasar TLX</a></td>
				<td>Kursus</td>
				<td>Indonesia</td>
				<td>Ikatan Alumni TOKI</td>
				<td><p>Belajar pemrograman kompetitif dasar dengan kurikulum yang disusun bertingkat, dan langsung latihan soal-soal melalui situs TLX.</p></td>
			</tr>
			<tr>
				<td><a href="/data/pemrograman-kompetitif-dasar.pdf">Pemrograman Kompetitif Dasar</a></td>
				<td>Buku</td>
				<td>Indonesia</td>
				<td>Ikatan Alumni TOKI</td>
				<td><p>Buku persiapan KSN, berfokus pada algoritma-algoritma yang masuk dalam silabus KSN.</p></td>
			</tr>
			<tr>
				<td><a href="https://cses.fi/book.html">Competitive Programmer's Handbook</a></td>
				<td>Buku</td>
				<td>Inggris</td>
				<td>Antti Laaksonen</td>
				<td><p>Buku pemrograman kompetitif yang dapat diunduh secara gratis. Buku ini ditulis untuk peserta tingkat internasional dan kompetisi mahasiswa sehingga Anda tidak perlu membaca keseluruhan buku ini.</p></td>
			</tr>
			<tr>
				<td><a href="https://cpbook.net/">Competitive Programming</a></td>
				<td>Buku</td>
				<td>Inggris</td>
				<td>Steven Halim</td>
				<td><p>Buku berbayar yang berisikan kumpulan algoritma beserta soal-soalnya. Anda dapat mengunduh versi pertama secara gratis. Buku ini juga dibuat untuk peserta tingkat internasional dan kompetisi mahasiswa sehingga Anda harus memilah-milah materi yang Anda pelajari.</p></td>
			</tr>
			<tr>
				<td><a href="https://visualgo.net/">VisuAlgo</a></td>
				<td>Situs</td>
				<td>Inggris</td>
				<td>NUS</td>
				<td><p>Situs yang berisikan visualisasi dari algoritma-algoritma populer.</p></td>
			</tr>
		</tbody>
	</table>
</div>

<div class="bp3-card subcontent">
	<h3>Lain-Lain</h3>
	<hr />
	<h4>Forum Diskusi</h4>
	<p>Media untuk diskusi tentang ide penyelesaian soal dan seputar kompetisi pemrograman.</p>
	<table class="table table-striped table-bordered">
		<thead>
			<tr>
				<th class="col-preparation-name">Sumber</th>
				<th class="col-preparation-by">Bentuk</th>
				<th>Catatan</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><a href="https://www.facebook.com/groups/olimpinfo/">Olimpiade Informatika Indonesia</a></td>
				<td>Grup Facebook</td>
				<td><p>Grup yang berisikan para alumni serta sesama peserta KSN. Anda dapat bertanya hal-hal yang berkaitan dengan KSN di sini.</p></td>
			</tr>
			<tr>
				<td><a href="http://line.me/ti/p/%40xtf2128i">Olimpiade Informatika Indonesia (@olimpinfo)</a></td>
				<td>Akun resmi LINE</td>
				<td><p>Akun yang membagikan informasi-informasi seputar KSN. Anda dapat bertanya kepada admin melalui <i>chat</i> personal.</p></td>
			</tr>
		</tbody>
	</table>

	<h4>Beasiswa dan Jalur Kuliah</h4>
	<p>Beberapa beasiswa dan jalur kuliah yang ditawarkan universitas dan perguruan tinggi di Indonesia bagi para peraih medali KSN.</p>
	<table class="table table-striped table-bordered">
		<thead>
			<tr>
				<th class="col-preparation-name">Program</th>
				<th>Oleh</th>
				<th>Catatan</th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td><a href="https://binus.ac.id/widia-scholarship/">Widia Scholarship</a></td>
				<td>BINUS University</td>
				<td><p>Beasiswa bagi pemenang olimpiade nasional dan internasional. Beasiswa berupa bantuan biaya-biaya kuliah.</p></td>
			</tr>
			<tr>
				<td><a href="https://admisi.ipb.ac.id/jalur-prestasi-internasional-dan-nasional-pin/">Jalur Prestasi Internasional dan Nasional</a></td>
				<td>Institut Pertanian Bogor</td>
				<td><p>Jalur penerimaan mahasiswa baru yang dikhususkan bagi siswa berprestasi.</p></td>
			</tr>
			<tr>
				<td><a href="http://um.ugm.ac.id/v.2015/id_home.php?l=aWRfcGFnZSMxNTEyMDUjSmFsdXIgUEJVQiMxI1Byb2dyYW0gU2FyamFuYQ==">Penelusuran Bibit Unggul Berprestasi</a></td>
				<td>Universitas Gadjah Mada</td>
				<td><p>Jalur penerimaan mahasiswa baru yang dikhususkan bagi penerima medali olimpiade nasional dan internasional.</p></td>
			</tr>
			<tr>
				<td><a href="https://www.ui.ac.id/pengantar-pendaftaran/program-olimpiade-sains.html">Program Olimpiade Sains</a></td>
				<td>Universitas Indonesia</td>
				<td><p>Jalur penerimaan mahasiswa baru yang dikhususkan bagi penerima medali olimpiade nasional dan internasional.</p></td>
			</tr>
			<tr>
				<td><a href="https://www.ui.ac.id/pengantar-pendaftaran/program-olimpiade-sains.html">Beasiswa Olimpiade Sains</a></td>
				<td>Universitas Multimedia Nusantara</td>
				<td><p>Bebas tes masuk bagi seluruh peserta KSN, serta pembebasan uang pangkal bagi pemeroleh medali.</p></td>
			</tr>
		</tbody>
	</table>
</div>

<?= $this->endSection() ?>
